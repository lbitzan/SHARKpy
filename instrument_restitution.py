...
"""
Module for instrument restitution and ground motion computation.

List of functions:
-------------------
- geophone_tf(trace: Trace, Gs: float, ... )
- apply_bb_tf_inv(trace: Trace)
- simulate_45Hz(trace: Trace)
- apply_geophone_tf(trace: Trace)
- apply_geophone_tf_inv(trace: Trace, transfer_function: np.ndarray, eps: float)
- hfilt_rmv_instr(trace: Trace, freq: int, detrend: str)
- compute_ground_motion(trace: Trace)
- calculate_Mlv(trace: Trace, distance: float, flag_obspy: bool)
- run_Mlv(catalog_1: str, catalog_2: str, year: int | list | None, month: int | list | None, day: int | list | None, hour: int | list | None, minute: int | list | None, stationdir: str, stationid: str, distance: float | None, drop_details: bool)
- read_magnitude_catalog(magnitude_catalog: str)
- plot_magnitudes(magnitude_catalog: pd.DataFrame, namespecifics: str)
- compare_sim45_vs_geophone(catalog_filename: str, datetime_start: str, datetime_end: str, stationdir_geophone: str, stationdir_bb: str, stationid_geophone: str, stationid_bb: str, savepath: str=None)
- test_geophone_simulation(catalog_filename: str, datetime_start: str, datetime_end: str, stationdir_geophone: str, stationdir_bb: str, stationid_geophone: str, stationid_bb: str, savepath: str=None)
...
def hfilt_rmv_instr(
    ...
    tr.filter('highpass', freq=freq)#, corners=corners, zerophase=True)
...
def calculate_Mlv(trace:        Trace,
    ...
    #A gain of 2080 should be applied to the Wood-Anderson trace and the input for the magnitude should be given in mm, usually.
    # Vertical local magnitude is after Bormann et al. GZF is calculated from a WA trace with normalised gain of 1.0 and an amplitude input in nanometers.
...
def run_Mlv(
    ...
    from instrument_restitution import apply_geophone_tf_inv, compute_ground_motion, calculate_Mlv
...
def read_magnitude_catalog(magnitude_catalog: str):
    # magcatafile     = '/results/magnitudes/'+magnitude_catalog
    # headers         = ['idx_eventcatalog','mlv','puretime']
    # cata            = pd.read_csv(rootdata+magcatafile, header=0, sep='\t', names=headers, index_col=False) #parse_dates=['date'])
    # cata['puretime']= pd.to_datetime(cata['puretime'])#, format='%d-%b-%Y %H:%M:%S')
    # cata.set_index(np.arange(len(cata)),inplace=True)
    cata                        = pd.read_pickle(magnitude_catalog)
    cata['idx_eventcatalog']    = cata.index
    cata.set_index(             np.arange(len(cata)), inplace=True)
    if flag_matching == True:
        headers_arr         = ['dstart','dend','baz','v_app','rmse']
        catalog_w_baz       = pd.read_csv(catalog_array, header=0, sep='\t', names=headers_arr)
        catalog_w_baz['puretime']     = pd.to_datetime(catalog_w_baz.dstart)
        cata['baz'] = catalog_w_baz['baz'].iloc[cata['idx_eventcatalog']].values
        cata.loc[cata['baz'] < 0, 'baz'] += 360

    magnitude_catalog = cata.copy()
    return magnitude_catalog

def plot_magnitudes(magnitude_catalog: pd.DataFrame,
                    namespecifics:     str,
                    nolegend:          bool = False):
    import  params_plot as      pplt
    from    kavutil     import  myplot, myplot_noleg
    cata                = magnitude_catalog.copy()
    mlv_mean            = cata['mlv'].mean()
    mlv_std             = cata['mlv'].std()
    cata['diff_mean']   = cata['mlv']- cata['mlv'].mean()

    if flag_matching == True:
        ax = plt.scatter(cata[cata.columns[1]],
                         cata['mlv'],
                         c      = cata['baz'],
                         s      = pplt.markersize,
                         cmap   = 'viridis',
                         alpha  = 0.5,
                         label  = 'Magnitude distribution,\nmean = '+str(np.round(mlv_mean,2)))
        cbar = plt.colorbar(label='Backazimuth [deg]')
    else:
        if 'date' in cata.columns:
            datecolumn = 'date'
        elif 'events' in cata.columns:
            datecolumn = 'events'
        else:
            print('No date column found in the magnitude catalog. Please check the column names.')
        ax = plt.scatter(cata[datecolumn],
                        cata['mlv'],
                        c      = 'darkblue',
                        s      = pplt.markersize,
                        alpha  = 0.5)
                        # label  ='Mlv distribution,\nmean = '+str(np.round(mlv_mean,2)))
    
    ax = plt.gca()
    ax.set_title('Magnitude distribution - total '+str(len(cata))+' events, mean = '+ str(np.round(mlv_mean,2)))
    ax.set_xlabel('Time')
    ax.set_ylabel('Local vertical magnitude Mlv')
    ax.set_ylim([np.min(cata['mlv']-1),np.max(cata['mlv'])+1])
    ...
    plt.savefig(rootdata+'/results/magnitudes/imgs/magn_'+namespecifics+'.png', dpi=400)
    plt.show()
    plt.close()
...
def run_Mlv_v2(
    ...
    from kav_init import hyp_dist, flag_matching, bbd_ids
    from obspy import Stream
    from instrument_restitution import apply_geophone_tf_inv, compute_ground_motion, calculate_Mlv
    ...
    # catalog         = cata[:len(selected_events)].copy()
...
def test_geophone_simulation(catalogfile: str,
    ...
    from kavutil import select_events_v2, read_catalog, define_ylim, retrieve_trace_data
...
# --- Running instrument restitution and magnitude computation ---
# if __name__ == '__main__':
    # from kavutil import select_events, add_colorbar_outside, read_catalogs_from_files, retrieve_trace_data, myplot

#     catalog_1, catalog_2, drop_details, flag_matching = 'catalogue_4.0_lb.txt','catalog_pl.txt', False, True
#     # catalog_1 = '/results/matching_test_ttol_ratiorange40_15sec.txt'
#     name4output = 'wBAZ_'

#     # mlv, selected_events, stream_events, stream_events_gndmtn, stream_events_restored = run_Mlv( catalog_1,catalog_2,2023,4,5,0, 48,'KAV04','c0939',None)
...
#     # cases = {'a': (2023, None, None, None, None, 'KAV11','c0941',None)}
#     cases = {'a': (2023, 4, 5, 0, 45, 'KAV11','c0941',None)}  
    
#     mlv_cases = {}

#     for i in cases:
#         print(i)
#         # mlv, selected_events, stream_events, stream_events_gndmtn, stream_events_restored = run_Mlv( catalog_1,catalog_2,*cases[i], flag_matching=flag_matching, drop_details=drop_details)
#         mlv, selected_events, stream_events, stream_events_gndmtn, stream_events_restored = run_Mlv( catalog_1,catalog_2,*cases[i], drop_details=drop_details)
...
#     pathmagresults = rootdata+'/results/magnitudes/magnitudes_catalog_4_lb/'
...
#     df.to_pickle(rootdata+'/results/magnitudes/magnitudes_catalog_4_lb.pkl')
#     plot_magnitudes(df, 'magnitudes_catalog_4_lb_new')


#     # mlv, selected_events, stream_events, stream_events_gndmtn, stream_events_restored = run_Mlv( catalog_1,catalog_2,2023,2,None,None, None,'KAV04','c0939',None)
...
if __name__ == '__main__':
    ...
    for cfile in os.listdir(os.path.join(rootdata,'results','compute_mags')):
        if cfile.endswith('.pkl'):
            ...
            _, _ = run_Mlv_v2(cfilepath,
                            ...
                            # datetime_start='2023-05-15',
                            # datetime_end='2023-05-25',
                            distance=None,
                            stationdir='KAV11',
                            stationid='c0941',
                            # stationdir='KAV04',
    ...
    # test_geophone_simulation(
    #     catalogfile=rootdata+'/results/catalogue_4.0_lb.txt')
    
    # compare_sim45_vs_geophone(rootdata+'/results/catalogue_4.0_lb.txt',
    #                           '2023-03-30T02:00:00',
    #                           '2023-03-30T03:15:00',
    #                           'KAV11','KAV00','c0941','c0bdd')

    # listofcatalogs = os.listdir(rootdata+'/results/cata_9.0_bbd.tbreaktest.lb/')
    # listofcatalogs.sort()
    # listofcatalogs.reverse()
    # listofcatalogs = [f for f in listofcatalogs if f.endswith('.pkl')]
    # listofcatalogs = listofcatalogs[9:20]; listofcatalogs.sort()
    # [print(f) for f in listofcatalogs]
    # breakpoint()

    # for cfile in listofcatalogs:
    #     cfilepath = os.path.join(rootdata+'/results/cata_9.0_bbd.tbreaktest.lb/', cfile)
    #     print(cfilepath)
    #     _,_ = run_Mlv_v2(cfilepath,
    #                         datetime_start='2023-02-01',
    #                         datetime_end='2023-12-01',
...
# # TODO: Add a function to add baz to final catalog, if flag_matching is True. Then include in plotting as color mapping.