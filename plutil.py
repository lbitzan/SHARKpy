...
import  time                as tm
import  datetime
# from    kavutil             import read_catalogs_from_files #, add_colorbar_outside, select_events, retrieve_trace_data
...
def plot_overview_slim(iplot):
    ...
    # in ini: kava1, kava2, d1spec, d2spec, d1f, d2f, kavaprod_slim, kavaprod2_slim, eventkava_slim, outputdir, cmapnamespec, kavatime, shift, shiftsp
    ...
    if (idx_wf[-1]+np.int64(ini.shift)) > len(pretr1.data):
        ...
        # timeplt, tspecplt, teventplt = [pd.to_datetime(customtime) + pd.to_timedelta(timearray-timearray[0], unit='s') for timearray in [timeplt, tspecplt, teventplt]]
        ...
        # timeplt,tspecplt, teventplt = [timearray.tz_localize('UTC').tz_convert('Pacific/Port_Moresby') for timearray in [timeplt, tspecplt, teventplt]]
        ...
        # ax0.ticklabel_format(axis='y', style='sci', scilimits=(0, 0))  # enforce sci notation for all ranges
        # ax0.yaxis.offsetText.set_fontsize(8)  # adjust font size of the 10^n label
        # ax0.set_zorder(1)  # Ensure this panel is on top
        # ax00.yaxis.set_major_formatter(ScalarFormatter(useMathText=True))
        # ax00.ticklabel_format(axis='y', style='sci', scilimits=(0, 0))
        # ax00.yaxis.offsetText.set_fontsize(8)  # adjust font size of the 10^n label
        ...
        # ax1.set_ylim(ini.fminbandpass[ini.stationdir[1]]//1, ini.fmaxbandpass[ini.stationdir[1]]+8)
        ax1.set_ylim(0, 65)
        yticks = np.arange(0, 63, 20)
        ax1.set_yticks(yticks)
        ax1.tick_params(axis='y', rotation=labelrot, labelsize=ini.fontsizelegend)
        ax11.tick_params(axis='y', rotation=labelrot, labelsize=ini.fontsizelegend)
        ...
        ax11.legend(loc='upper right', fontsize=ini.fontsizelegend)
        ax1.grid(alpha=0.5)
        ...
        ax2.tick_params(axis='y', rotation=labelrot, labelsize=ini.fontsizelegend)
        ax22.tick_params(axis='y', rotation=labelrot, labelsize=ini.fontsizelegend)
        ...
        ax22.legend(loc='upper right', fontsize=ini.fontsizelegend)
        ax2.grid(alpha=0.5)

        # Panel 3: SHARK index + detections + fill_between
        ax3 = fig.add_subplot(gs[3], sharex=ax0)
        # Custom fill_between: allow for custom threshold or mask
        # fill_mask = sharkplt >= ini.kavaprodemp if hasattr(ini, 'kavaprodemp') else sharkplt > 0
        # ax3.fill_between(tspecplt, 0, sharkplt, where=fill_mask, facecolor='tab:blue', alpha=0.4)
...
def plot_spectra(selected_events:   pd.DataFrame,
    ...
    from kavutil  import myplot, retrieve_trace_data, define_ylim
...
def plot_revise(datetime_start: str,
                datetime_end:   str,
                catalogflie:    str,):
    from kav_init import rootdata, stationdir, stationid
