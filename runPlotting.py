...
flag_bandedratio = False # True: compute amplitude ratio based on particular frequency bands, False: compute amplitude ratio based on whole frequency band

outputlabel  = 'newtrigger' # specify label attached to output directory to label specific input values
...
from kavutil       import rolling_stats, hl_envelopes_idx
...
for imoi in moi:
    ...
    # soi     = 45*60  # ---> set zero to run full hour analysis // minutes * 60 = seconds
    ...
    for idoi in doi:
        ...
        outputcatalog= 'D:/data_kavachi_both/catalogue_lb.txt'                                                         # <--- set path and name for output catalogue for larger data set on external hard drive
        if not os.path.exists(outputdir):
            os.makedirs(outputdir)

        # --- Computations ------------------------------------------------------------------------------------------------- 
        # --- Assign traces to variables
        tr1          = st1[0]
        tr2          = st2[0]
        ...
        ratioemp     = 18                        # empirical value [close station/remote station]
        conflim      = 0.2                       # confidence limit for amplitude ratio trigger
        # --- Set characteristic KaVA Index for normalization --------------------------------------------------------------
        # kava2emp     = 100                      # empirical value [close station]
        # kava1emp     = 15                       # empirical value [remote station]
        # kavaratioemp = 3.5                      # empirical value [close station/remote station]
        ...
        winshift    = .5 / tr1.stats.delta                                              # define window shift [s] / dt
        win         = int(4 / tr1.stats.delta)                                          # length of intervall window /dt - in [s]
        itr         = np.arange(win/2, len(tr1.data)-win/2-int(shift), winshift, dtype=int)   # define centered indices of intervals
        iampl       = np.arange(len(itr), dtype=int)                                    # define indices of amplitudes array

        # --- Allocate arrays
        tampl       = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitude times
        ampl1       = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitudes array
        ampl2       = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitudes array
        ampl1var    = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitudes array
        ampl2var    = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitudes array
        diff2_1     = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitude ratio array
        diff2_1var  = np.zeros_like(iampl,dtype=float)                                  # Allocate amplitude ratio std array
        normdiff2_1 = np.zeros_like(iampl,dtype=float)                                  # Allocate normalized amplitude ratio array



        # --- Alternative amplitude ratio computation -----------------------------------------------------------------------
        if flag_bandedratio == False:
            tr2.filter('bandpass',freqmin=fmin, freqmax=fmax, zerophase=True)
            tr1.filter('bandpass',freqmin=fmin, freqmax=fmax, zerophase=True)

            ampl2       = rolling_stats(np.abs(tr2.data), np.amax, window=10)
            ampl1       = rolling_stats(np.abs(tr1.data), np.amax, window=10)

            ampl2var    = rolling_stats(np.abs(tr2.data), np.std, window=10)  # add shift to window size
            ampl1var    = rolling_stats(np.abs(tr1.data), np.std, window=10)

            diff2_1     = ampl2[:-int(shift)] /ampl1[int(shift):]

            tampl       = time

        # --- compute amplitude ratio based on particular frequency bands --------------------------------------------------------------
        if flag_bandedratio:
            tr2ratio    = tr2.filter('bandpass',freqmin=f2min2, freqmax=f2max2, zerophase=True)
            tr1ratio    = tr1.filter('bandpass',freqmin=f1min1, freqmax=f1max1, zerophase=True)

            ampl2       = rolling_stats(np.abs(tr2ratio.data), np.amax, window=10)
            ampl1       = rolling_stats(np.abs(tr1ratio.data), np.amax, window=10)

            ampl2var    = rolling_stats(np.abs(tr2ratio.data), np.std, window=10)  # add shift to window size
            ampl1var    = rolling_stats(np.abs(tr1ratio.data), np.std, window=10)
        ...
        # --- Compute ratio output -------------------------------------------------------------------------------------------------
        # if flag_bandedratio == False:
        #     for i in iampl:
        #         # 2: close array
        #         ampl2[i]      = np.amax(np.abs(tr2.data[itr[i]-int(win/2) : itr[i]+int(win/2)+1]))
        #         ampl2var[i]   = np.std( np.abs(tr2.data[itr[i]-int(win/2) : itr[i]+int(win/2)+1]))
        #         # 1: remote array with shifted trace
        #         ampl1[i]      = np.amax(np.abs(tr1.data[itr[i]-int(win/2)+int(shift) : itr[i]+int(win/2)+1+int(shift)]))
        #         ampl1var[i]   = np.std( np.abs(tr1.data[itr[i]-int(win/2)+int(shift) : itr[i]+int(win/2)+1+int(shift)]))
        #         # 2/1: ratio of amplitudes (close/remote)
        #         diff2_1[i]    = ampl2[i]/ampl1[i]
        #         diff2_1var[i] = ampl2var[i] + ampl1var[i]
        #         # time of amplitudes (without shift)
        #         tampl[i]      = time[itr[i]]

        normdiff2_1   = diff2_1/ratioemp

        # --- Compute KaVA Index -------------------------------------------------------------------------------------------------
        d1          = st1[0]
        d2          = st2[0]
        d1.filter('bandpass', freqmin=fmin, freqmax=fmax, zerophase=True)
        d2.filter('bandpass', freqmin=fmin, freqmax=fmax, zerophase=True)

        # --- Spectrograms
        nfft        = int(sr)  # int(sr)
        noverlap    = int(sr*.75) #= int(sr/2)
        cspec   = plt.get_cmap('viridis')                           # colormap for spectrogram
        d1spec, d1f, d1t, cax = plt.specgram(d1.data, NFFT=nfft, Fs= int(sr), noverlap=noverlap, detrend='none', scale_by_freq=True, cmap=cspec)
        plt.close()
        d2spec, d2f, d2t, cax = plt.specgram(d2.data, NFFT=nfft, Fs= int(sr), noverlap=noverlap, detrend='none', scale_by_freq=True, cmap=cspec)
        plt.close()
        # --- KaVA Index
        kava2           = (np.sum(d2spec[f2min1:f2max1,:], axis=0)+np.sum(d2spec[f2min2:f2max2,:],axis=0)) / (np.sum(d2spec[fmin:f2min1,:],axis=0) + np.sum(d2spec[f2max1:f2min2,:],axis=0) + np.sum(d2spec[f2max2:fmax,:],axis=0))
        kava1           = np.sum(d1spec[f1min1:f1max1,:],axis=0) / (np.sum(d1spec[fmin:f1min1,:],axis=0) +  np.sum(d1spec[f1max1:fmax,:],axis=0))
        ...
        kavaprod2       = rolling_stats(kavaprod, np.amax, window=5)    # max envelope of kavaprod
        
        # --- Implement event trigger based on amplitude ratio
        # Add one to normdifftrig when normdiff2_1 differs from one within confidence limits of 15%
        normdifftrig    = np.zeros_like(normdiff2_1)
        normdifftrig    = np.where(np.abs(normdiff2_1 - 1) <= conflim, normdifftrig + 1, normdifftrig)
        ...
        print("Number of indices in kava time: ", len(d2t))
        print("Number of indices in ampl time: ", len(tampl))

        if len(d2t) > len(tampl):
            ''' Reduce data point frequency of kava product to match amplitude ratio data point frequency '''
            idx4trigtime    = d2t[:-np.int64(shiftsp)].searchsorted(tampl)
            ratiotrigger    = normdifftrig
            kavatrigger     = kavaprodtrig[idx4trigtime]

        elif len(d2t) < len(tampl):
            ''' Reduce data point frequency of amplitude ratio to match kava product data point frequency '''
            idx4trigtime    = tampl.searchsorted(d2t[:-np.int64(shiftsp)])
            kavatrigger     = kavaprodtrig
            ratiotrigger    = normdifftrig[idx4trigtime]
        else:
        ...
        print("Number of indices in idx4trigtime: ", len(idx4trigtime))
        print("Number of indices in kava trigger: ", len(kavatrigger))
        print("Number of indices in ratio trigger: ", len(ratiotrigger))
        print("Number of indices in combined trigger: ", len(combtrigger))

        eventmarker     = np.zeros_like(combtrigger)
        eventmarker[1:] = np.where(combtrigger[1:] - combtrigger[:-1] == 1, eventmarker[1:] +1, eventmarker[1:])
        eventmarker[0]  = 0

        # eventratio      = normdiff2_1[np.where(eventmarker == 1)]

        if len(d2t) > len(tampl):
            # eventmarkertimes= tampl[        np.where(eventmarker == 1)]
            eventmarkertimes= d2t[:-np.int64(shiftsp)][idx4trigtime[np.where(eventmarker == 1)]]
            eventratio      = normdiff2_1[  np.where(eventmarker == 1)]
            eventkava       = kavaprod2[    idx4trigtime[np.where(eventmarker == 1)]] # kavaprod2 is the max envelope of kavaprod
        elif len(d2t) < len(tampl):
            # eventmarkertimes= tampl[        idx4trigtime[np.where(eventmarker == 1)]]
            eventmarkertimes= d2t[:-np.int64(shiftsp)][np.where(eventmarker == 1)]
            eventratio      = normdiff2_1[  idx4trigtime[np.where(eventmarker == 1)]]
            eventkava       = kavaprod2[    np.where(eventmarker == 1)]

        print("number of indices in eventmarker: ", len(eventmarker))
        print("Number of events in eventmarkertimes: ", len(eventmarkertimes))
        print("Number of indices in eventratio ", len(eventratio))
        print("\n last indices in idx4trigtime: ", idx4trigtime[-5:])
        ...
        if write_flag:
            ...
            for i in np.arange(len(eventmarkertimes)):
                seconds4catalogue   = eventmarkertimes[i]
        ...
        if plot_flag:
            ...
            # hoi = [0] # set hours of interest ------------ for testing only ------------
            ...
            for h in hoi:
                ...
                for s in np.arange(0, 60*60, sectionlen):
                    ...
                    idxevent    = np.where((eventmarkertimes >= h*60*60 + s) & (eventmarkertimes < h*60*60 + s + sectionlen))

                    # idxenv      = np.where((tenv  >= h*60*60 + s) & (tenv  < h*60*60 + s + sectionlen))
                    # idxenv1     = np.where((tenv1 >= h*60*60 + s +shifttime) & (tenv1 < h*60*60 + s + sectionlen + shifttime))
                    # idxkavasmooth = np.where((tkavasmooth   >= h*60*60 + s) & (tkavasmooth   < h*60*60 + s + sectionlen))
                    ...
                    if (idxtr[0][-1]+int(shift)) > len(tr1.data):
                        ...
                        diff2_1plt     = diff2_1[    idxampl]
                        diff2_1varplt  = diff2_1var[ idxampl]
                        normdiff2_1plt = normdiff2_1[idxampl]
                        normdifftrigplt= normdifftrig[idxampl]
                        ...
                        kavaprodtrigplt= kavaprodtrig[idxsp]

                        eventmarkertimesplt = eventmarkertimes[idxevent] - h*60*60 - m*60
                        ...
                        color   = 'tab:red'
                        ax1.plot(timeplt, tr2plt, color=color, alpha=.7, linewidth=0.4)
                        ...
                        ax1twin = ax1.twinx()
                        color   = 'tab:blue'
                        ax1twin.plot(timeplt, tr1plt[0,:], color=color, alpha=.7, linewidth=0.4)
                        ...
                        # --- plot normalised amplitude ratio ------------------------------------------------------------------------------
                        cax2 = ax2.scatter( tamplplt, normdiff2_1plt, c = normdiff2_1plt, cmap='PRGn',vmin=0, vmax=2, alpha=.5, label='Norm. amplitude ratio') # cmap = 'RdYlBu' 'RdPu' 'set1' 'nipy_spectral'
                        ax2.fill_between(tamplplt, 0, np.max(normdiff2_1plt), where=normdifftrigplt==1, facecolor='orange',  alpha=0.4, label='Ratio trigger')
                        ax2.fill_between(tspecplt, 0, np.max(normdiff2_1plt), where=kavaprodtrigplt==1, facecolor='tab:red', alpha=0.1, label='KaVA trigger')
                        ax2.set_ylabel('Normalised\n amplitude ratio\n' + subdir[1] + ' / ' + subdir[0], rotation=90, labelpad=10)
                        ax2.set_yticks(np.arange(0, np.amax(normdiff2_1plt),1))
                        ax2.legend(loc='upper right'); ax2.grid(alpha=0.3)
                        add_colorbar_outside(cax2, ax2)
                        
                        # --- Plot Advanced KaVA Index (KaVA Product) ----------------------------------------------------------------------
                        ax34.plot(tspecplt, kavaprodplt, color='k', alpha=0.8, linewidth=1, label='Adv KaVA Index')
                        ax34.plot(tspecplt, kavaprod2plt, color='b', alpha=0.5, linewidth=1,linestyle='--', label='Adv KaVA Index envelope')
                        ax34.fill_between(tspecplt, 0, np.max(kavaprodplt), where=kavaprodtrigplt==1, facecolor='tab:red', alpha=0.1, label='KaVA trigger')
                        ax34.fill_between(tamplplt, 0, np.max(kavaprodplt), where=normdifftrigplt==1, facecolor='orange', alpha=0.4, label='Ratio trigger')
                        ax34.scatter(eventmarkertimesplt, np.amax(kavaprodplt)/2*np.ones_like(eventmarkertimesplt), c='r', marker=(5,2), label='Event trigger')
                        ax34.set_ylabel('Adv KaVA Index', rotation=90, labelpad=10)
