...
def runPlotCatalog(rrange = None, trigbreak = None):
    '''
    ...
    from kavutil import org_days, get_data, compute_simple_ratio, compute_banded_ratio, compute_kava, rolling_stats, fusetriggers, add_colorbar_outside #, myplot
    ...
    start_time = tm.time()
    print('start time:        ', start_time, 'sec')
    print('Triggerresttime:   ', triggerresttime, 'sec')
    print('Ampl. ratio range: ', str(ratiorange[0]),' to ' + str(ratiorange[1])+' sec')
    print('Threshold Shark:   ', str(kavaprodemp))
    ...
    for imoi in moi:
        ...
        for idoi in doi:
            ...
            outputdir    = rootouts+outputlabel+'/'+outputlabel+'20' + '{year}'.format(year=yoi) +'{:02d}'.format(imoi)+'/'+outputlabel+'20'+ '{year}'.format(year=yoi) + '{:02d}'.format(imoi) + '{:02d}'.format(idoi) + str(triggerresttime) +'/' # <--- set path outputdir for  larger data set on external hard drive
