...
def compute_banded_ratio(x1, x2, freqbands, shifttime=2.25, stats=['KAV04','KAV11'], zerophase=True, corners=2, window=20):
    '''
...
def retrieve_trace_data(selected_events, stationdir, stationid):
    ...
    for index, event in selected_events.iterrows():
        if 'puretime' in selected_events.columns:
            event_time = event['puretime']
        elif 'date' in selected_events.columns:
            event_time = event['date']
        else:
            print('No time column found in the selected events DataFrame.')
...
def fusetriggers(kavatrigger, ratiotrigger, ratiotime, triggerresttime = 20):
    ...
    # ax1.plot(ratiotime[snip1:snip2], aend[snip1:snip2], 'r-',alpha=.5, label= 'aend')
    # ax1.fill_between(ratiotime[snip1:snip2], 0, a1[snip1:snip2], color='b', alpha=.4, label= 'shark')
    # # ax1.plot(ratiotime[snip1:snip2], a1[snip1:snip2], 'b:', label= 'a1', alpha=.5)
    # ax1.plot(ratiotime[snip1:snip2], b1[snip1:snip2], color='orange', label= 'ratio',   alpha=.5)
    # ax1.plot(ratiotime[snip1:snip2], step1[snip1:snip2], 'g--', label= 'event', alpha=.5)
    # ax1.legend(loc='best')
    # ax2.plot(ratiotime[snip1:snip2], switchrec[snip1:snip2], 'r-', label= 'switch')
    # ax3.plot(ratiotime[snip1:snip2], counterrec[snip1:snip2], 'r-', label= 'counter')
    # plt.show()
    # breakpoint()
    eventmarker = step1
    # breakpoint()
...
def select_events_v2(df:             pd.DataFrame,
    ...
    print('Select recorded events between', datetime_start, 'and', datetime_end)
    return selected_events
...
def get_days_list(datetime_start: str,
    ...
    #TODO: remove certain days from the list like 4th of July
    return dayslist
